...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const lazims = await prisma.lazim.findMany({
        where: { userId: user.id },
        orderBy: { createdAt: 'desc' },
      });
      return ok(res, { lazims });
    }
    ...
    if (req.method === 'POST') {
      ...
      const lazim = await prisma.lazim.create({
        data: {
          ...parsed.data,
          userId: user.id,
        },
    ...
    const updated = await prisma.lazim.update({
      where: { id },
      data: { completed },
    });

    ok(res, { lazim: updated });
  } catch (error) {
    console.error('lazim error', error);
